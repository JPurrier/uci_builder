...
class Buildetwork(object):
    ...
    def create_epg_stat_bnd(self,xl_file):
        ...
        set_of_tenants = set()
        set_of_AppProf = [] #this will read all App profiles irrespective of tenant I think that it will work still but there might be a better way to do this - ask J
        epg_encap = ''
        epg_paths_tDN = ''
        epg_protpaths_tDN = ''
        results = []
        ...
        for tenant in set_of_tenants:
            ...
            for index, row in epg_stat_bnd_sheet.iterrows():
                if row['status'] == 'ignored':
                    ...
                    if row['tenant'] == tenant:
                        for AppProf in set_of_AppProf:
                            fvAp = SubElement(self.fvTenant,'fvAp',{'name':row['app_profile'],'status':'modified'})
                            #for index, row in epg_stat_bnd_sheet.iterrows():
                            if row['tenant'] == tenant and row['app_profile'] == AppProf:
