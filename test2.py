    ...
    def create_subject(self,xl_file):
        ...
        for tenant in set_of_tenants:
            ...
            for contr_sbj in set_of_contract:
                ...
                print(tenant,contr_sbj,set_of_subject)
                for sbj_sbj in set_of_subject:
                    vzSubj = SubElement(vzBrCP,'vzSubj',{'consMatchT':'AtleastOne',
                                                               'name':sbj_sbj,'nameAlias':'','prio':'unspecified',
                                                                'provMatchT':'AtleastOne',
                                                                'status':'','targetDscp':'unspecified'})
                    sbj_flt_dn = ''
                    for index, row_filt in subject_sheet.iterrows():
                        if row_filt['tenant'] == tenant and row_filt['contract'] == contr_sbj and row_filt['name'] == sbj_sbj and pd.isnull(row_filt['status']):
                            sbj_flt_dn = 'uni/tn-' + tenant + '/brc-' + contr_sbj + '/subj-' + sbj_sbj + '/rssubjFiltAtt-' + row_filt['filter']
                            print(sbj_flt_dn)
