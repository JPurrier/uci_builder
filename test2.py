...
class Buildetwork(object):
    ...
    def __init__(self):
        ...
        self.sw_prof = 'switch_profile'
        self.aaep = 'aaep'
        self.aaep_domain = 'aeep_domain_association'
        ...
        for tenant in set_of_tenants:
            ...
            for app_prof in set_of_AppProf:
                ...
                for index, row_app in epg_sheet.iterrows():
                    #print(tenant,app_prof,row_app['name'])
    ...
    def create_epg_stat_bnd(self,xl_file):
        ...
        for tenant in set_of_tenants:
            ...
            for app_prof in set_of_AppProf:
                ...
                print (set_of_EPG)
                for epg_name in set_of_EPG:
                    fvAEPg = SubElement(fvAp,'fvAEPg',{'matchT':'AtleastOne','name':epg_name,'status':'modified'})
                    for index, row_epg in epg_stat_bnd_sheet.iterrows():
                        if row_epg['tenant'] == tenant and row_epg['app_profile'] == app_prof and row_epg['name'] == epg_name and pd.isnull(row_epg['status']):
                            if pd.isnull(row_app['right_node_id']):    
                                epg_encap = 'vlan-' + str(int(row_epg['encap_vlan_id']))
                                epg_paths_tDN = 'topology/pod-' + str(int(row_epg['pod_id'])) + '/paths-' + str(int(row_epg['left_node_id'])) + '/pathep-[eth' + str(row_epg['access_port_id']) + ']'
                                fvRsPathAtt = SubElement(fvAEPg,'fvRsPathAtt',{'descr':'','encap':epg_encap,'instrImedcy':'lazy','mode':row_epg['mode'],'status':'','tDn':epg_paths_tDN})
                            else:
                                epg_encap = 'vlan-' + str(row_epg['encap_vlan_id'])
                                epg_protpaths_tDN = 'topology/pod-' + str(int(row_epg['pod_id'])) + '/protpaths-' + str(int(row_epg['left_node_id'])) + '-' + str(int(row_epg['right_node_id'])) + '/pathep-[' + row_epg['interface_policy_group'] + ']'
                                fvRsPathAtt = SubElement(fvAEPg,'fvRsPathAtt',{'descr':'','encap':epg_encap,'instrImedcy':'lazy','mode':row_epg['mode'],'primaryEncap':'unknown','tDn':epg_protpaths_tDN})
            results.append(root)
            #print(tenant,set_of_AppProf)
            set_of_AppProf.clear()
        for result in results:
    ...
    def create_sw_prof(self,xl_file):
        # sw_prof_int_prof SpreadSheet
        excel_file = xl_file
        sw_prf_sheet = pd.read_excel(excel_file,sheet_name=self.sw_prof)
        blkname = ''
        results = []
        root = Element('polUni')
        # root.set('version', '1.0')
        root.append(Comment('Generated by JTools UCI Builder'))
        infraInfra = SubElement(root, 'infraInfra')
        for index, row in sw_prf_sheet.iterrows():
            if row['switch_profile_type'] == 'leaf' and pd.isnull(row['status']):
                blkname = 'bl' + str(int(row['from_node_id'])) + str(int(row['to_node_id']))
                infraNodeP =  SubElement(infraInfra,'infraNodeP',{'annotation':'','descr':'','name':row['name'],'status':''})
                infraLeafS =  SubElement(infraNodeP,'infraLeafS',{'annotation':'','descr':'','name':row['switch_selector'],'type':'range'})
                infraNodeBlk =  SubElement(infraLeafS,'infraNodeBlk',{'annotation':'','descr':'','from_':str(int(row['from_node_id'])),'name':blkname,'to_':str(int(row['to_node_id']))})
        results.append(root)
        for result in results:
            print(ExcelImporter().prettify(result))
        return results
    ...
    def create_aaep(self,xl_file):
        excel_file = xl_file
        raw_aaep_sheet = pd.read_excel(excel_file,sheet_name=self.aaep)
        aaep_sheet = raw_aaep_sheet.sort_values(by=['status'])
        results = []
        root = Element('polUni')
        # root.set('version', '1.0')
        root.append(Comment('Generated by JTools UCI Builder'))
        infraInfra = SubElement(root, 'infraInfra')
        for index, row in aaep_sheet.iterrows():
            if pd.isnull(row['status']):
                infraAttEntityP = SubElement(infraInfra, 'infraAttEntityP',{'descr':str(row['description']),'name':str(row['name']),'status':''})
        results.append(root)
        for result in results:
            print(ExcelImporter().prettify(result))
        return results
    ...
    def create_aaep_domain(self,xl_file):
        excel_file = xl_file
        raw_aaep_dom_sheet = pd.read_excel(excel_file,sheet_name=self.aaep_domain)
        aaep_dom_sheet = raw_aaep_dom_sheet.sort_values(by=['aaep_name'])
        set_of_aaep = set()
        results = []
        for index, row in aaep_dom_sheet.iterrows():
            if pd.notnull(row['status']):
                continue
            else:
                set_of_aaep.add(row['aaep_name'])
        set_of_aaep = sorted(set_of_aaep)
        root = Element('polUni')
        # root.set('version', '1.0')
        root.append(Comment('Generated by JTools UCI Builder'))
        infraInfra = SubElement(root, 'infraInfra')
        for aaep_row in set_of_aaep:
            infraAttEntityP = SubElement(infraInfra, 'infraAttEntityP',{'name':aaep_row,'status':'modified'})
            for index, row in aaep_dom_sheet.iterrows():
                if row['aaep_name'] == aaep_row and row['domain_type'] == 'physical' and pd.isnull(row['status']):
                    tDn = 'uni/phys-' + row['domain_name']
                    infraRsDomP = SubElement(infraAttEntityP, 'infraRsDomP',{'tDn':tDn})
                elif row['aaep_name'] == aaep_row and row['domain_type'] == 'external_l3' and pd.isnull(row['status']):
                    tDn = 'uni/l3dom-' + row['domain_name']
                    infraRsDomP = SubElement(infraAttEntityP, 'infraRsDomP',{'tDn':tDn})   
                elif row['aaep_name'] == aaep_row and row['domain_type'] == 'vmm_vmware' and pd.isnull(row['status']):
                    tDn = 'uni/vmmp-VMware/dom-' + row['domain_name']
                    infraRsDomP = SubElement(infraAttEntityP, 'infraRsDomP',{'tDn':tDn})     
        results.append(root)
        for result in results:
            print(ExcelImporter().prettify(result))
    ...
    def call_all_aci_elements(self):
        ...
        #Buildetwork().create_sw_prof(xl_file)
        #Buildetwork().create_sw_prof_intf_prof(xl_file)
        #Buildetwork().create_aaep(xl_file)
        Buildetwork().create_aaep_domain(xl_file)
