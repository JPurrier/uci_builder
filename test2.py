    ...
    def create_int_pol_group(self,xl_file):
        ...
        for index, row in int_pol_grp_sheet.iterrows():
            ...
            if row['interface_policy_group_type'] == 'vPC' and pd.isnull(row['status']):
                infraAccBndlGrp = SubElement(infraFuncP, 'infraAccBndlGrp',{'desc':row['description'],'lagT':'node','name':row['name'],'status':''})
                ...
                infraAccPortGrp = SubElement(infraFuncP,'infraAccPortGrp',{'desc':row['description'],'name':row['name'],'status':''})
