    ...
    def create_epg_stat_bnd(self,xl_file):
        ...
        for index, row in epg_stat_bnd_sheet.iterrows():
            if row['status'] == 'ignored':
                ...
                set_of_tenants.add(row['tenant'])
                print('tenant : {} status {}'.format(row['tenant'], row['status']))
        ...
        for index, row in epg_stat_bnd_sheet.iterrows():
            if row['status'] == 'ignored':
                ...
                set_of_AppProf.append(row['app_profile'])
                
                print('app profile : {} status: {}'.format(row['app_profile'], row['status'] ))
                #list_of_tenants = list(epg_stat_bnd_sheet['tenant'])
                #list_of_apps = list(epg_stat_bnd_sheet['app_profile'])

        #print(set_of_tenants)
        print('set_app_profile : {}'.format(set_of_AppProf))
        #print(list_of_tenants)
        #print(list_of_tenants)
        print('---' * 15)
        print('set of tenants {}'.format(set_of_tenants))
        for tenant in set_of_tenants:
            root = Element('polUni')
            # root.set('version', '1.0')
            root.append(Comment('Generated by JTools UCIs Builder'))
            self.fvTenant = SubElement(root, 'fvTenant', {'name': tenant, 'status': 'modified'})
            last_app = ''
            print('working on tenant {}'.format(tenant))
            for index, row in epg_stat_bnd_sheet.iterrows():
                if row['status'] == 'ignored':
                    continue
                else:
                    if row['tenant'] == tenant:
                        for app_prof in set_of_AppProf:
                            if row['app_profile'] == app_prof:
                                if last_app != app_prof:
                                    fvAp = SubElement(self.fvTenant,'fvAp',{'name':row['app_profile'],'status':'modified'})
                                    last_app = row['app_profile']
                                    print('app prov {}'.format(fvAp))
                                else:
                                    continue
                            else:
                                continue
                        #for index, row in epg_stat_bnd_sheet.iterrows():
                        fvAEPg = SubElement(fvAp,'fvAEPg',{'matchT':'AtleastOne','name':row['name'],'status':'modified'})
                        if pd.isnull(row['right_node_id']):    
                            epg_encap = 'vlan-' + str(row['encap_vlan_id'])
                            epg_paths_tDN = 'topology/pod-' + str(row['pod_id']) + '/paths-' + str(row['left_node_id']) + '/pathep-[eth' + str(row['access_port_id']) + ']'
                            fvRsPathAtt = SubElement(fvAEPg,'fvRsPathAtt',{'descr':'','encap':epg_encap,'instrImedcy':'lazy','mode':row['mode'],'status':'','tDn':epg_paths_tDN})

                        else:
                            print(row['left_node_id'])
                            print(row['right_node_id'])
                            epg_encap = 'vlan-' + str(row['encap_vlan_id'])
                            epg_protpaths_tDN = 'topology/pod-' + str(row['pod_id']) + '/protpaths-' + str(row['left_node_id']) + '-' + str(int(row['right_node_id'])) + '/pathep-[' + row['interface_policy_group'] + ']'
                            fvRsPathAtt = SubElement(fvAEPg,'fvRsPathAtt',{'descr':'','encap':epg_encap,'instrImedcy':'lazy','mode':row['mode'],'primaryEncap':'unknown','tDn':epg_protpaths_tDN})
            results.append(root)
